...
| Runner widoczny w panelu, ale bez metod | Pusty katalog `Modules\` albo żadna paczka nie zawiera publicznej klasy implementującej `IRunnerMethod`. Sprawdź w logu wpisy `Add method:`; poziom `Debug` dokłada `Unppack module:` i `Check dll:`. |
