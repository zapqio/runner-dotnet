...
| --- | --- |
| Usługa nie startuje, w Dzienniku zdarzeń `You must install .NET to run this application` albo `The framework 'Microsoft.WindowsDesktop.App', version '10.0.0' was not found` | Paczka jest framework-dependent i wymaga zainstalowanego **.NET Desktop Runtime 10 (x64)** — sam .NET Runtime nie wystarczy. `winget install Microsoft.DotNet.DesktopRuntime.10` albo instalator z [dotnet.microsoft.com](https://dotnet.microsoft.com/download/dotnet/10.0); sprawdzisz przez `dotnet --list-runtimes`. |
